...
# If Homebrew is installed but not yet on this shell's PATH (fresh install,
# or a shell that hasn't sourced zprofile), load it before deciding anything.
...
# Only install Homebrew if it's genuinely absent.
if ! command -v brew >/dev/null; then
